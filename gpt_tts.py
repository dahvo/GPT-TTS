...
book_name = "Frankenstien"
language = "en"
embedding_file_path = "audiobook_lady.json"
df = pd.read_csv(f"{book_name}_processed.csv")

pbar = tqdm(total=len(df), desc="Processing Text")
...
speaker_embedding, gpt_cond_latent = load_speaker_embedding(embedding_file_path)

# Call process_sentences with the loaded embeddings
full_wav_data, subtitles = process_sentences(df['Sentence'].tolist(), model, language, speaker_embedding, gpt_cond_latent, sample_rate, sample_width)


pbar.close()

# Specify the output file path for the synthesized speech
output_wav_file_path = f'{book_name}.wav'
subtitles_output_path = f'{book_name}_captions.srt'

create_srt(subtitles, subtitles_output_path)
# Save the full WAV file
with wave.open(output_wav_file_path, 'wb') as wav_file:
    wav_file.setnchannels(1)  # Assuming mono audio
    wav_file.setsampwidth(2)  # Assuming 2 bytes per sample (16-bit audio)
    wav_file.setframerate(24000)  # Replace with the actual framerate of your TTS model
    final_wav_data_bytes = full_wav_data.tobytes()
    wav_file.writeframes(final_wav_data_bytes)
